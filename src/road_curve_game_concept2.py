...
class ReverseCarGame:
    def __init__(self):
        ...
        # L'IA de la voiture (volontairement nulle pour le défi !)
        self.car_target_x = self.car_x
        self.car_ai_speed = 1.5  # Plus lente
        ...
        self.road_curve_speed = 2.5  # Vitesse de courbure plus élevée !
        self.max_curve = 6  # Courbure maximum augmentée
    ...
    def handle_road_control(self, keys):
        ...
        # Contrôle plus fin avec A/D (plus réactif)
        if keys[pygame.K_a]:
            self.road_direction = max(-self.max_curve, self.road_direction - 1.0)
        if keys[pygame.K_d]:
            self.road_direction = min(self.max_curve, self.road_direction + 1.0)
    ...
    def update_cars_ai(self):
        """IA volontairement nulle des voitures pour plus de défi !"""
        for car in self.cars:
            # L'IA vise le centre de la route mais avec retard et imprécision
            road_center = self.get_road_center_at_y(car['y'] + 40)  # Regarde plus loin
            
            # Ajoute de l'imprécision à l'IA
            imprecision = random.randint(-15, 15)
            car['target_x'] = road_center + imprecision
            
            # Déplacement plus lent et moins précis
            diff = car['target_x'] - car['x']
            if abs(diff) > self.car_ai_speed:
                move = self.car_ai_speed * 0.7  # Encore plus lent
                car['x'] += move if diff > 0 else -move
            
            # L'IA réagit parfois en retard (1 chance sur 3 de ne pas bouger)
            if random.random() < 0.3:
                pass  # Ne bouge pas ce frame
            
            # Vérifie si la voiture est sur la route
            road_center_actual = self.get_road_center_at_y(car['y'])
            road_left = road_center_actual - self.road_width // 2
            road_right = road_center_actual + self.road_width // 2
    ...
    def draw_ui(self):
        ...
        # Objectif mis à jour
        objective_text = self.small_font.render("🎯 L'IA est nulle ! Guidez-la avec la route !", True, YELLOW)
        screen.blit(objective_text, (WIDTH - 380, 10))
