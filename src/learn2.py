...
class CarGame:
    ...
    def get_simple_state(self):
        ...
        # Chaque capteur peut avoir 3 valeurs, et on a 5 capteurs
        # Donc 3^5 = 243 états possibles maximum
        state = 0
        for i, sensor_val in enumerate(discrete_sensors):
            state += sensor_val * (3 ** i)
        
        return min(state, 242)  # Limite à 242 pour éviter les dépassements
    ...
    def step(self, action):
        ...
        if self.total_time > 500:  # Plus court pour forcer l'apprentissage
            done = True
            # Récompense finale basée sur le score
